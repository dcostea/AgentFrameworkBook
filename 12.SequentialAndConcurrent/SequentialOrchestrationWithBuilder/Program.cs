...
Workflow workflow = AgentWorkflowBuilder
    .CreateSequentialBuilderWith(environmentAgent, safetyAgent, motorsAgent)
    .WithName("RobbyMission")
...
await foreach (WorkflowEvent evt in run.WatchStreamAsync())
{
  switch (evt)
  {
    ...
    case WorkflowOutputEvent output:
      ...
      Microsoft.Extensions.AI.ChatMessage? lastMessage = messages?.LastOrDefault();

      if (!string.IsNullOrWhiteSpace(lastMessage?.Text))
      {
        ColorHelper.PrintColoredLine($"\n[WORKFLOW OUTPUT] {lastMessage.Text}\n", ConsoleColor.Yellow);
      }
