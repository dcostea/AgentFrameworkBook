...
// Reads sensors and routes to the right specialist. Routing conditions live in the handoff reasons.
...
var fireDetectorAgent = chatClient.AsAIAgent("""
    ...
    When finished, hand off to the MotorsAgent.
    """,
    "FireDetectorAgent",
...
var workflow = AgentWorkflowBuilder.CreateHandoffBuilderWith(environmentAgent)
  // Fan-out: the router dispatches to ONE specialist. Each routing condition now lives in the
  // handoffReason argument (migrated from the EnvironmentAgent's instructions). Per-edge reasons
  // require the single-target WithHandoff overload.
  .WithHandoff(environmentAgent, fireDetectorAgent, "Transfer if the temperature is above 60°C (possible fire).")
  .WithHandoff(environmentAgent, rainDetectorAgent, "Transfer if the droplet level is High (rain detected).")
  .WithHandoff(environmentAgent, motorsAgent, "Transfer if conditions are safe (temperature normal and no heavy rain).")
  // Fan-in: both detectors converge on the MotorsAgent with a shared reason.
