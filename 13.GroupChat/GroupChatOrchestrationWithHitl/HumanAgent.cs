...
public sealed class HumanAgent : AIAgent
{
  ...
  private static ChatMessage CreateApprovalDecisionMessage(IEnumerable<ChatMessage> messages)
  {
    if (messages.Any(message => message.Text?
      .Contains(nameof(ExecutionDecision.EXECUTED), StringComparison.Ordinal) == true))
    {
    ...
    {
      return new ChatMessage(ChatRole.Assistant, 
         $"HUMAN {nameof(ExecutionDecision.APPROVED)}: the proposed move sequence looks good. Execute it now.");
    }
  
    return new ChatMessage(ChatRole.Assistant, 
      $"HUMAN {nameof(ExecutionDecision.DENIED)}: please revise the proposed move sequence.");
  }
