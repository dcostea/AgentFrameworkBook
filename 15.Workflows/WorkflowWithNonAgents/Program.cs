...
await foreach (WorkflowEvent evt in run.WatchStreamAsync())
{
  ...
  {
    ColorHelper.PrintColoredLine($"{completed.ExecutorId}: {completed.Data}", ConsoleColor.Yellow);
  }
  else if (evt is ExecutorFailedEvent failed)
  {
    ColorHelper.PrintColoredLine($"Executor '{failed.ExecutorId}' failed: {failed.Data}", ConsoleColor.Red);
  }
  else if (evt is WorkflowErrorEvent error)
  {
    ColorHelper.PrintColoredLine($"[WORKFLOW ERROR] {error.Exception?.ToString() ?? "Unknown workflow error."}", ConsoleColor.Red);
  }
}
